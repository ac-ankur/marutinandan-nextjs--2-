...
// Kept conservative: MAX_PIXELS controls grid density for optimal performance.
const MAX_PIXELS = 36;
const KEYFRAME_STEPS = 2;
...
function PixelSwap({
  ...
  const stopAnimations = useCallback(() => {
    ...
    pixelRefs.current.forEach(pixel => {
      ...
      if (pixel) pixel.style.willChange = 'auto';
    });
  ...
  useEffect(() => {
    ...
    rafId = requestAnimationFrame(() => {
      ...
      // 1. Single Master Clone created once to avoid cloning 3000+ DOM nodes on main thread
      const masterClone = source.cloneNode(true);
      masterClone.classList.remove('invisible');
      masterClone.dataset.visible = 'true';
      masterClone.setAttribute('inert', '');
      masterClone.setAttribute('aria-hidden', 'true');
      masterClone.style.animation = 'none';

      // 2. Single pass to strip internal CSS animations
      const animatedNodes = masterClone.querySelectorAll('*');
      for (let i = 0; i < animatedNodes.length; i++) {
        animatedNodes[i].style.animation = 'none';
      }

      // 3. Fast lightweight cloning for pixel windows
      frozenGrid.pixels.forEach((pixel, index) => {
        ...
        const clone = masterClone.cloneNode(true);
        ...
        const windowAnim = pixelElement.animate(keyframes.window, timing);
        const contentAnim = content.animate(keyframes.content, timing);

        windowAnim.onfinish = () => {
          if (pixelElement) pixelElement.style.willChange = 'auto';
        };
        animationsRef.current.push(windowAnim, contentAnim);
      });
